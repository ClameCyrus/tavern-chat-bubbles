// ============================================================
// 芙林_聊天气泡_专属版 v9
// 内核：通用版 v14.4（流式三通道 / 开关安全写入 / 回读校验 / 表情包库 / 外观配置）
...
function initFlynnChatBubbles() {
  ...
  function avatarHTML(kind, rawName) {
    ...
    const img = src
      ? `<img class="fhb-av" src="${escapeAttr(src)}" alt="" loading="lazy" onerror="this.style.display='none'">`
      : '';
...
.fhb-avatar{position:absolute;inset:0;width:100%;height:100%;border-radius:50%;overflow:hidden;border:1.5px solid var(--fhb-gold);box-shadow:0 3px 10px rgba(0,0,0,.25);z-index:2;}
...
.fhb-vine{position:absolute;inset:-8px;width:calc(100% + 16px);height:calc(100% + 16px);overflow:visible;pointer-events:none;z-index:3;}
